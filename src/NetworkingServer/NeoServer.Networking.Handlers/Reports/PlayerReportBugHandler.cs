...
public class PlayerReportBugHandler(IReportBugRepository reportBugRepository, IGameCreatureManager creatureManager, GameConfiguration gameConfiguration) : PacketHandler
{
    public override async void HandleMessage(IReadOnlyNetworkMessage message, IConnection connection)
    {
        ...
        if (reportBugEntity != null)
        {
            ...
            {
                connection.Send(new TextMessagePacket($"You can only report a bug every {gameConfiguration.Report.reportMaxTime} minutes.", TextMessageOutgoingType.Small));
                return;
            }
        }
