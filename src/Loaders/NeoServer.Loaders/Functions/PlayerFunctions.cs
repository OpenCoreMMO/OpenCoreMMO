...
public class PlayerFunctions : LuaScriptInterface, IPlayerFunctions
{
    ...
    public void Init(LuaState luaState)
    {
        ...
        RegisterMethod(luaState, "Player", "getHouses", LuaPlayerGetHouses);
        RegisterMethod(luaState, "Player", "getGuid", LuaPlayerGetGuid); 
        
        // Outfit functions for newtype command
        RegisterMethod(luaState, "Player", "getOutfit", LuaPlayerGetOutfit);
        RegisterMethod(luaState, "Player", "setOutfit", LuaPlayerSetOutfit);
    }
    ...
    private static int LuaPlayerGetMoney(LuaState luaState)
    {
        ...
        var totalMoney = player.GetTotalMoney(_coinTypeStore);
        Lua.PushNumber(luaState, totalMoney);
        return 1;
    }
    ...
    private static int LuaPlayerRemoveMoney(LuaState luaState)
    {
        ...
        var amount = (ulong)Lua.ToNumber(luaState, 2);
        var success = _coinTransaction.RemoveCoins(player, amount);
        
        Lua.PushBoolean(luaState, success);
        return 1;
    }

    private static int LuaPlayerGetHouses(LuaState luaState)
    {
        // player:getHouses()
        var player = GetUserdata<IPlayer>(luaState, 1);
        if (player == null)
        ...
        // Create a Lua table to hold the houses
        Lua.NewTable(luaState);
        
        // TODO: In a full implementation, this would query the database
        // or a house service to find houses owned by the player.
        // For now, return an empty table since we don't have a house service yet.
        
        return 1;
    }

    private static int LuaPlayerGetGuid(LuaState l)
    {
        var player = GetUserdata<IPlayer>(l, 1);
        if (player == null)
        {
            Console.WriteLine("[PlayerFunctions] LuaPlayerGetGuid: player is null");
            Lua.PushNil(l);
            return 1;
        }

        Console.WriteLine($"[PlayerFunctions] LuaPlayerGetGuid: Player {player.Name} returning GUID: {player.Id}");
        Lua.PushNumber(l, player.Id);
        return 1;
    }

    private static int LuaPlayerGetOutfit(LuaState luaState)
    {
        var player = GetUserdata<IPlayer>(luaState, 1);
        if (player == null)
        {
        ...
        // Create a Lua table with outfit properties
        Lua.NewTable(luaState);
        
        Lua.PushNumber(luaState, outfit.LookType);
        Lua.SetField(luaState, -2, "lookType");
        
        Lua.PushNumber(luaState, outfit.Head);
        Lua.SetField(luaState, -2, "lookHead");
        
        Lua.PushNumber(luaState, outfit.Body);
        Lua.SetField(luaState, -2, "lookBody");
        
        Lua.PushNumber(luaState, outfit.Legs);
        Lua.SetField(luaState, -2, "lookLegs");
        
        Lua.PushNumber(luaState, outfit.Feet);
        Lua.SetField(luaState, -2, "lookFeet");
        
        Lua.PushNumber(luaState, outfit.Addon);
        Lua.SetField(luaState, -2, "lookAddons");
        
        return 1;
    }

    private static int LuaPlayerSetOutfit(LuaState luaState)
    {
        var player = GetUserdata<IPlayer>(luaState, 1);
        if (player == null)
        {
        ...
        // Get outfit table from second parameter
        if (Lua.Type(luaState, 2) != LuaType.Table)
        {
            Lua.PushBoolean(luaState, false);
            return 1;
        }

        // Extract outfit values from table
        Lua.GetField(luaState, 2, "lookType");
        var lookType = (ushort)Lua.ToNumber(luaState, -1);
        Lua.Pop(luaState, 1);

        Lua.GetField(luaState, 2, "lookHead");
        var head = (byte)Lua.ToNumber(luaState, -1);
        Lua.Pop(luaState, 1);

        Lua.GetField(luaState, 2, "lookBody");
        var body = (byte)Lua.ToNumber(luaState, -1);
        Lua.Pop(luaState, 1);

        Lua.GetField(luaState, 2, "lookLegs");
        var legs = (byte)Lua.ToNumber(luaState, -1);
        Lua.Pop(luaState, 1);

        Lua.GetField(luaState, 2, "lookFeet");
        var feet = (byte)Lua.ToNumber(luaState, -1);
        Lua.Pop(luaState, 1);

        Lua.GetField(luaState, 2, "lookAddons");
        var addon = (byte)Lua.ToNumber(luaState, -1);
        Lua.Pop(luaState, 1);

        // Create new outfit and change player outfit
        var newOutfit = new Outfit();
        newOutfit.Change(lookType, head, body, legs, feet, addon);
        
        player.ChangeOutfit(newOutfit);
