...
public class AttackValidation(IMapTool mapTool, IMap map, PvPConfiguration pvpConfiguration)
{
    ...
    private bool IsProtected(IPlayer playerAggressor, IPlayer playerTarget)
    {
        ...
        if (playerAggressor.VocationType == 0 || playerTarget.VocationType == 0)
        {
            return true;
        }

        if (playerAggressor.Skull is Skull.Black && playerAggressor.GetSkull(playerTarget) is Skull.None)
        {
            return true;
        }
