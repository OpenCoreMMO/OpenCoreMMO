...
public static class FindFirstItemByServerIdQuery
{
    public static (IItem ItemFound, IContainer Container, byte SlotIndex) Find(IContainer onContainer, ushort serverId)
    {
        ...
        while (containers.TryDequeue(out var container))
        {
            ...
            foreach (var containerItem in container.Items)
            {
                ...
                if (containerItem.ServerId == serverId) return (containerItem, container, slotIndex);
