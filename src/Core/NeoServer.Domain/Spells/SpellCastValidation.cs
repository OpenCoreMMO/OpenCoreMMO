...
public class SpellCastValidation(IMapTool mapTool)
{
    public Result CanBeCastBy(ICombatActor caster, IThing target, ISpell spell)
    {
        ...
        if (spell.IsSelfTarget) target = caster;

        var result = spell.CanCast(caster, target);
        if (result.Failed) return result;

        if ((caster.Tile?.ProtectionZone ?? false) && (spell.PrimaryGroup.Name == "attack" ||
                                                       spell.SecondaryGroup.Name == "Attack" ||
                                                       spell.IsAggressive))
        ...
        var casterHasNoTarget = spell.HasCooldownGroup(MagicGroupConstants.ATTACK) && caster.CurrentTarget is null;
