...
public class WorldLight
{
    ...
    public byte LightLevel { get; private set; } = LIGHT_LEVEL_DAY;
    ...
    private void UpdateLightLevel()
    {
        ...
        {
            case Period.Sunrise:
                LightLevel = (byte)Math.Min(LightLevel + transitionStep, LIGHT_LEVEL_DAY);
                if (LightLevel == LIGHT_LEVEL_DAY)
                    _currentPeriod = Period.Day;
                break;

            case Period.Sunset:
                LightLevel = (byte)Math.Max(LightLevel - transitionStep, LIGHT_LEVEL_NIGHT);
    ...
    {
        EventAggregator.Invoke(new WorldLightChangedEvent(LightLevel));
    }
