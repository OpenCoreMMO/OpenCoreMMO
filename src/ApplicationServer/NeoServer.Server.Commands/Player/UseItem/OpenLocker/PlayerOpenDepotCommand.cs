...
public class PlayerOpenDepotCommand(IPlayerUseService playerUseService,
    IPlayerDepotItemRepository playerDepotItemRepository, IItemFactory itemFactory, LockerManager lockerManager): ICommand
{
    ...
    private IContainer LoadDepotChest(IPlayer player, IItem container)
    {
        ...
        if (lockerManager.IsDepotLoaded(player.Id))
        {
