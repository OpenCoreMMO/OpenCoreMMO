...
public class PlayerOpenMailInboxCommand(
    ...
    LockerManager lockerManager) : ICommand
{
    ...
    private IContainer LoadMailInbox(IPlayer player, IItem container)
    {
        ...
        if (locker.Items.Count == 2 &&
            locker.Items[1].Metadata.Attributes.GetAttribute(ItemTypeAttribute.Type) != "mailbox")
        ...
        if (lockerManager.IsMailboxLoaded(player.Id)) return (IContainer)locker.Items[1];
