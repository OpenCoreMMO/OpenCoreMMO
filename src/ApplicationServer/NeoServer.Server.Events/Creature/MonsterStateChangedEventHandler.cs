...
public class MonsterStateChangedEventHandler(ICreatureGameInstance creatureGameInstance)
    : IApplicationEventHandler<MonsterStateChangedEvent>
{
    public void Handle(MonsterStateChangedEvent @event)
    {
        ...
        if (@event.ToState == MonsterState.Sleeping) creatureGameInstance.TryRemove(@event.Monster.CreatureId);
    }
}